...
from codes.openai.unet import UNetModel as unet
from codes.openai_1ch.unet import UNetModel as unet_k
from codes.diffusion import Model
from codes.OrigDataset import SingleDataset
...
class Diffusion(object):
    ...
    def sample(self,image_folder, init_noise=None):
        cls_fn = None
        args, config = self.args, self.config
        model = unet(in_channels=3, model_channels=64, out_channels=3,
                                num_res_blocks=2, attention_resolutions=[16, 8])
        ckpt = getattr(args, 'ckpt', './model/model_char.pth')
        state = torch.load(ckpt, map_location=self.device)
        # stain_removal形式: {"model": state_dict, "epoch": ..., ...}
        if isinstance(state, dict) and 'model' in state and 'epoch' in state:
            state = state['model']
        model.load_state_dict(state)
        model.to(self.device)
        model.eval()
        self.sample_sequence(model, cls_fn,image_folder, init_noise=init_noise)


    def sample_sequence(self, model, cls_fn=None,image_folder=None, init_noise=None):
        args, config = self.args, self.config
        ...
        deg = args.deg
        H_funcs = None
        from functions.svd_replacement import Inpainting as Inpainting
        ...
        loaded = Image.open(combine_img_path_binary)
        ...
        for x_orig, classes in pbar:
            ...
            if init_noise is not None:
                # ページ全体で共有した1枚のノイズマップから該当パッチ位置を
                # 切り出したものを使う（パッチ間ムラ対策: 隣接パッチの初期ノイズに
                # 相関を持たせ、独立乱数によるパッチ境界の不整合を緩和する）
        ...
        #         tvu.save_image(
        #             (x[i][j]), os.path.join(image_folder, f"{args.result}/{idx_so_far + j}_{i}.png"), normalize=True,var_type=(-1,1)
        #         )
        #         mask_inv = (mask-1)*(-1)
        #         x_only_mask = x[i][j].detach().cpu()*mask.detach().cpu() + x_orig.detach().cpu()*mask_inv.detach().cpu()
        #         x_only_mask = x_only_mask*2-1
