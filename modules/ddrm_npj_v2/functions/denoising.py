...
def efficient_generalized_steps(x, seq, model, b, H_funcs, y_0, sigma_0,  device, etaB, etaA, etaC, \
                                cls_fn=None, classes=None, mask=None, x_orig=None,args=None):
    with torch.no_grad():
        ...
        # if len(U_t_y.view(-1)) <= len(singulars.view(-1)):
            #y_var
        Sig_inv_U_t_y = U_t_y / singulars[:U_t_y.shape[-1]] #non-zero
        # else:
        #     Sig_inv_U_t_y = U_t_y[0][:len(singulars) ]/ singulars
        #initialize x_T as given in the paper
        largest_alphas = compute_alpha(b, (torch.ones(x.size(0)) * seq[-1]).to(x.device).long())   #a_999
        #largest variance of q(x_t-1 | x_t, x0)
        ...
        for i, j in tqdm(zip(reversed(seq), reversed(seq_next))):
            ...
            at = compute_alpha(b, t.long())    #shpae[batch,1,1,1]
            at_next = compute_alpha(b, next_t.long())
            
            xt = xs[-1].to(device)
            # pad = int((1000-i)*ij/1000)
            # mask_pad = erase(torch.zeros_like(xt), ij-pad,ij-pad,mask_size+pad*2,mask_size+pad*2,1)
            """COVER OR UNCOVER"""
            

            if args.masking:
                #i >= 50:
                if i >= args.mask_timesteps:
                    # 背景画像を条件として使用する場合は、これをやる##########################
                    # background = Image.open(args.back_ground).convert("RGB")
                    # background = background.resize((xt.shape[3], xt.shape[2]))  # (width, height)
                    # background_tensor = transforms.ToTensor()(background).to(args.device)
                    # # マスク適用時に背景を条件として使用
                    # xt = xt * mask.to(args.device) + (1 - mask.to(args.device))*background_tensor
                    #########################################################
                    ...
                    #xtは画像とか特徴マップのテンソル
                    #maskはマスクのテンソル
                    #mask_timestepまではxtとmaskを掛け合わせて、mask_timestep以降はmaskを掛け合わせない
                    #maskの0は文字、1は背景
                    xt = xt*mask.to(args.device)
            ...
            """savg proce"""
            # xt_mod_test = H_funcs.V(Vt_xt_mod_next)
            # xt_test = (at_next.sqrt()[0, 0, 0, 0] * xt_mod_test).view(*x.shape)
            # tvu.save_image(
            #             xt_test, f"{args.image_folder}/test_{i}.png", normalize=True, value_range=(-1,1),
            #         )
           
            
            #Vt_xt_mod_next = torch.zeros_like(Vt_xt_mod_next)
            ...
            x0_preds.append(x0_t.to(args.device))
            xs.append(xt_next.to(args.device))


    return xs, x0_preds,classes