"""
DDRM推論パイプライン（ddrm_npj_v2 モジュール用）。

処理フロー:
  1. 文字マスク生成: --mask_dir指定時はgenerate_mask_csc.pyのCSC黒文字マスク
     （npj提案手法、v2クラスタリング）、未指定時は大津二値化（比較ベースライン）
...
使い方（npj提案手法・CSCマスク使用）:
    ...
    python scripts/pipeline_ddrm.py \
        ...
        --output_dir ../../tmp_work/ddrm_result_v2/200003803 \
        ...
        --device     cuda:0 \
        --wandb_run_name  ddrm-infer-csc-v2-gpu0
...
def _patch_inference(image_bgr: np.ndarray, mask01: np.ndarray,
                     ...
                     patch_size: int = 128, overlap: int = 32,
                     shared_noise: bool = False) -> np.ndarray:
    ...
    # ページ全体で共有する1枚のノイズマップ。各パッチはここから対応位置を切り出して
    # 初期ノイズとして使う（隣接パッチ間のノイズに空間的相関を持たせ、独立乱数由来の
    # パッチ境界の不整合＝ムラを緩和する）。
    page_noise = None
    if shared_noise:
        page_noise = torch.randn(1, 3, H + patch_size, W + patch_size, device=runner.device)
    ...
    for i, (y, x) in enumerate(coords, 1):
        ...
        init_noise = None
        if page_noise is not None:
            init_noise = page_noise[:, :, y:y+patch_size, x:x+patch_size]

        orig_size = runner.config.data.image_size
        runner.config.data.image_size = patch_size
        runner.sample(patch_dir, init_noise=init_noise)
...
def process_page(img_path: str, out_dir: str, runner: Diffusion, tmp_dir: str,
                 patch_size: int, overlap: int, wb=None, mask_dir: str = None,
                 shared_noise: bool = False) -> None:
...
def main():
    ...
    parser.add_argument('--shared_noise', action='store_true',
                        help='パッチ間ムラ対策: ページ全体で1枚の共有ノイズマップから'
                             '各パッチの初期ノイズを切り出す（独立乱数を使わない）')
