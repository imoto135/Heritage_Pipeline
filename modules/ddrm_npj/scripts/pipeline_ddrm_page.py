"""
DDRM Stage2 ページ推論パイプライン（stain_removal論文の2段階手法）。
...
処理フロー（論文 Section IV-B）:
  ...
  Stage2（本スクリプト）:
    patch結果画像 → 128×128にリサイズ
    → 文字マスク（--mask_dir指定時はCSC黒文字マスク=npj提案手法、
       未指定時は大津二値化=npj論文中の比較ベースライン）
...
使い方（npj提案手法・CSCマスク使用）:
    ...
    python scripts/pipeline_ddrm_page.py \
        ...
        --output_dir       ../../tmp_work/ddrm_page_result/200003803 \
        --mask_dir         ./data/csc_output/200003803 \
        --ckpt             ../../modules/stain_removal/model/model_page/ckpt_epoch0400.pth \
        --device           cuda:0 \
        --wandb_run_name   ddrm-page-infer-csc-ep400-gpu0-200003803
