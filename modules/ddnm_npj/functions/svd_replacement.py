...
class GeneralH(H_functions):
    def mat_by_vec(self, M, v):
        ...
        return torch.matmul(M,v).view(1,-1)
    ...
    def __init__(self, H, device):
        ...
        self._Vt = self._V.transpose(1, 2)
        self._Ut = self._U.transpose(1, 2)

        ZERO = 1e-3
        self._singulars[self._singulars < ZERO] = 0
        # print(self._singulars)
...
#Weak
class Stain2(H_functions):
    def __init__(self, channels, img_dim, missing_indices, low_s_indices, device):
        self.channels = channels
        self.img_dim = img_dim
        self.missing_indices = missing_indices
        self.low_s_indices = low_s_indices
       
        self.kept_indices = torch.Tensor([i for i in range(channels * img_dim**2) if i not in torch.cat((missing_indices,low_s_indices),dim=0)]).to(device).long()
        self._singulars = torch.ones(channels * img_dim**2 - (missing_indices.shape[0]+self.low_s_indices.shape[0])).to(device)
        self._sigulars_low = torch.ones(channels * img_dim**2 - (self.kept_indices.shape[0]+self.missing_indices.shape[0])).to(device)
        self._sigulars_low *= 0.998
        self._singulars = torch.cat((self._singulars,self._sigulars_low),dim=0)
    def V(self, vec):
        temp = vec.clone().reshape(vec.shape[0], -1)
      
        out = torch.zeros_like(temp)
        out[:, self.kept_indices] = temp[:, :self.kept_indices.shape[0]]
        out[:, self.low_s_indices] = temp[:, self.kept_indices.shape[0]:(self.kept_indices.shape[0]+self.low_s_indices.shape[0])]
        out[:, self.missing_indices] = temp[:, (self.kept_indices.shape[0]+self.low_s_indices.shape[0]):]
    ...
    def Vt(self, vec):
        ...
        out[:, self.kept_indices.shape[0]: (self.kept_indices.shape[0]+self.low_s_indices.shape[0])] = temp[:, self.low_s_indices]
        out[:, (self.kept_indices.shape[0]+self.low_s_indices.shape[0]):] = temp[:, self.missing_indices]
        return out
...
class keep(H_functions):
    def __init__(self, channels, img_dim, low_s_indices, device):
        self.channels = channels
        self.img_dim = img_dim
        self.low_s_indices = low_s_indices
       
        self.kept_indices = torch.Tensor([i for i in range(channels * img_dim**2) if i not in low_s_indices]).to(device).long()
        self._singulars = torch.ones(channels * img_dim**2 - (self.low_s_indices.shape[0])).to(device)
        self._sigulars_low = torch.ones(channels * img_dim**2 - (self.kept_indices.shape[0])).to(device)
        self._sigulars_low *= 0.8
        self._singulars = torch.cat((self._singulars,self._sigulars_low),dim=0)
    def V(self, vec):
        temp = vec.clone().reshape(vec.shape[0], -1)
...
class Inpainting(H_functions):
    def __init__(self, channels, img_dim, missing_indices, device):
        ...
        self.missing_indices = missing_indices
        # kept_indices を set ではなく mask ベースで高速計算
        total = channels * img_dim**2
        all_mask = torch.ones(total, dtype=torch.bool)
        all_mask[missing_indices.cpu()] = False
        self.kept_indices = torch.where(all_mask)[0].to(device)
    ...
    def H(self, vec):
        # unmasked ピクセルのみ抽出して返す (B, C, H, W) -> (B, kept)
        temp = vec.clone().reshape(vec.shape[0], self.channels, -1).permute(0, 2, 1).reshape(vec.shape[0], -1)
        return temp[:, self.kept_indices]

    def Ht(self, vec):
        # H†: unmasked ピクセルを元の位置に戻し、masked ピクセルは 0 にする
        # vec: (B, C, H, W) or (B, kept) → (B, C, H, W)
        batch = vec.shape[0]
        if vec.dim() == 4:
            flat = vec.reshape(batch, self.channels, -1).permute(0, 2, 1).reshape(batch, -1)
            src = flat[:, self.kept_indices]
        else:
            src = vec.reshape(batch, -1)
        out = torch.zeros(batch, self.channels * self.img_dim**2, device=vec.device)
        out[:, self.kept_indices] = src
        return out.reshape(batch, self.channels, self.img_dim, self.img_dim)

#Denoising
class Denoising(H_functions):
    def __init__(self, channels, img_dim, device):
        self._singulars = torch.ones(channels * img_dim**2, device=device)*0.1
...
class Colorization(H_functions):
    def __init__(self, img_dim, device):
        ...
        H = torch.Tensor([[1,1,1]]).to(device)
        self.U_small, self.singulars_small, self.V_small = torch.svd(H, some=False)
        self.Vt_small = self.V_small.transpose(0, 1)
    ...
    def singulars(self):
        return self.singulars_small.repeat(self.img_dim**2)*0.3
